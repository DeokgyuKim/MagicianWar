...
Meteor_FireEff2::Meteor_FireEff2(ID3D12Device* device, ID3D12GraphicsCommandList* cmdLst, Renderer* pRenderer, Object* pParent)
...
Meteor_FireEff2::~Meteor_FireEff2()
{
}

void Meteor_FireEff2::BuildComponent()
{
	Component* pComponent = new Transform(XMFLOAT3(0.3f, 0.3f, 0.06f), XMFLOAT3(0.f, 0.f, 0.f), XMFLOAT3(0.f, 0.f, 4.5f));
	...
	pComponent = new Mesh(m_pDevice, m_pCmdLst, m_pRenderer->GetHeap(), "Eff_Arrow_Effect01");
...
void Meteor_FireEff2::AddTexturesName()
{
	m_lstTextureName.push_back("Lava");
	m_lstTextureName.push_back("Noise4");
}

void Meteor_FireEff2::BuildShaders()
{
...
void Meteor_FireEff2::LateUpdate(const float& fTimeDelta)
{
...
void Meteor_FireEff2::Render(const float& fTimeDelta, int _instanceCount)
{
