...
BlackHole_Body::BlackHole_Body(ID3D12Device* device, ID3D12GraphicsCommandList* cmdLst, Renderer* pRenderer, Object* pParent)
...
BlackHole_Body::~BlackHole_Body()
{
}

void BlackHole_Body::BuildComponent()
{
	Component* pComponent = new Transform(XMFLOAT3(0.01f, 0.01f, 0.01f), XMFLOAT3(0.f, 0.f, 0.f), XMFLOAT3(20.f, 2.f, 10.f));
	...
	pComponent = new Mesh(m_pDevice, m_pCmdLst, m_pRenderer->GetHeap(), "Eff_HalfSphere1");
	m_mapComponent["Mesh"] = pComponent;
	pComponent = new MaterialCom("BlackHole", XMFLOAT4(1.f, 1.f, 1.f, 1.f), XMFLOAT4(1.f, 1.f, 1.f, 1.f), XMFLOAT4(0.f, 0.f, 0.f, 0.f));
	m_mapComponent["Material"] = pComponent;

	m_MaterialIndex = dynamic_cast<MaterialCom*>(m_mapComponent["Material"])->GetMaterialIndex();
}

void BlackHole_Body::AddTexturesName()
{
	m_lstTextureName.push_back("FireMakeLong");
	m_lstTextureName.push_back("Noise3");
}

void BlackHole_Body::BuildShaders()
{
...
void BlackHole_Body::LateUpdate(const float& fTimeDelta)
{
...
void BlackHole_Body::Render(const float& fTimeDelta, int _instanceCount)
{
